...
export class Repo extends EventEmitter<RepoEvents> {
  ...
  /**
   * Creates a new document with custom ID and returns a handle to it. The initial value of the document is an
   * empty object `{}` unless an initial value is provided. Its documentId is generated by the
   * system. we emit a `document` event to advertise interest in the document.
   */
  createWithId<T>(docId: string, initialValue?: T): DocHandle<T> {
    // Generate a new UUID and store it in the buffer
    const { documentId } = parseAutomergeUrl(
      `automerge:${bs58check.encode(
        new Uint8Array(Uuid.parse(stringToUuidV4(docId)))
      )}` as AutomergeUrl
    )
    const handle = this.#getHandle<T>({
      documentId,
  ...
  /**
   * Retrieves a document by id or creates it. It gets data from the local system,
   * but also emits a `document` event to advertise interest in the document.
   */
  findOrCreate<T>(
    /** The human-readable ID of the handle to retrieve */
    id: string
  ): DocHandle<T> {
    const { documentId } = parseAutomergeUrl(
      `automerge:${bs58check.encode(
        new Uint8Array(Uuid.parse(stringToUuidV4(id)))
      )}` as AutomergeUrl
    )
